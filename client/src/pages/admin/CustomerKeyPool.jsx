...
export default function AdminCustomerKeyPool() {
  ...
  const load = () => {
    apiFetch('/admin/customer-key-pool').then(setKeys).catch(() => {});
  ...
  const handleAddKeys = async () => {
    ...
    try {
      await apiFetch('/admin/customer-key-pool', {
        method: 'POST',
        body: JSON.stringify({
  ...
  const handleDeleteKey = async (id) => {
    ...
    try {
      await apiFetch(`/admin/customer-key-pool/${id}`, { method: 'DELETE' });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await apiFetch('/admin/customer-key-pool-bulk', {
        method: 'DELETE',
  ...
  const handleClearSold = async () => {
    if (!confirm('Delete ALL sold keys? This cannot be undone.')) return;
    try {
      const data = await apiFetch('/admin/customer-key-pool-bulk', {
        method: 'DELETE',
  ...
  return (
    <AdminLayout>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h1 className="page-title">Customer Key Pool</h1>
          <p style={{ color: 'var(--text-secondary)' }}>
            Manage activation keys for customers. Keys auto-deliver when you approve orders.
          </p>
      ...
      {filtered.length === 0 ? (
        <div className="empty-state"><h3>No keys found</h3><p>Add customer activation keys to the pool</p></div>
      ) : (
